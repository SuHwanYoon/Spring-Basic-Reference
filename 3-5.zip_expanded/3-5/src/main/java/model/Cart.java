...
public class Cart {
	...
	public void push(ItemSet pushItemSet) {//장바구니에 과일정보한줄+갯수를 넣는다
		...
		Fruit pushedItem = pushItemSet.getItem();
		int pushedItemId = pushedItem.getItem_id();//상품번호읽음
		for(ItemSet cartItemSet: this.itemList) {
			Fruit cartItem = cartItemSet.getItem();
			int cartItemId = cartItem.getItem_id();//cartItemId이미 장바구니에 있는 상품번호
